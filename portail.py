...
urlOfRandomNumber="https://www.calculator.net"
...
def randomNumberOfHTTPGet():
    ...
    modem.send_command(CHTTPACT.write_cmd([urlOfRandomNumber, 443]))
    print("command sent")
    ...
    while ("CHTTPACT: 0" not in reponse[-1]) and (startOfAnswer not in reponse[-1]):
        print(reponse[-1])
        reponse.append(getLineInString())
    if (startOfAnswer not in reponse[-1]):
        return reponse[-1]
    else:
        return None
...
def sendSMStoNum(num):
    initSMS()
    modem.send_command(CMGS.write_cmd(num))
    time.sleep(1)
    modem.send_data(b'message'+eof)
    print("sent")
    print(getLineInString())
    print("fini")
...
def getLineInString():
    return modem._ser.readline().decode().strip("\\n")#.strip("\\r")[2:-1]
...
if __name__ == "__main__":
    with SerialModem(DEVICE_PORT, DEVICE_PIN) as modem:
        ...
        elif answerRandom.removesuffix(startOfAnswer)[:1]:
            print(answerRandom)
    print("fini")